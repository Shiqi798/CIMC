...
void Key_Debounce_Tick(void)
{
	...
	static const uint32_t key_gpio[KEY_COUNT] = {GPIOE, GPIOB, GPIOC, GPIOA};
	static const uint32_t key_pins[KEY_COUNT] = {GPIO_PIN_7, GPIO_PIN_0, GPIO_PIN_4, GPIO_PIN_6};
	uint8_t real_level[KEY_COUNT];

	for (uint8_t i = 0; i < KEY_COUNT; i++)
	{
		// 双重采样
		uint8_t t1 = gpio_input_bit_get(key_gpio[i], key_pins[i]);
		uint8_t t2 = gpio_input_bit_get(key_gpio[i], key_pins[i]);
